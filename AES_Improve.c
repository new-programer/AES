	...
	for (int i = 0; i < 4; ++i)
	{
		for (int j = 0; j < 4; ++j)
		{
			...
			case 0:
				...
				//M矩阵的第一行乘以行移后tem矩阵的第一列
				state[i][j] = GMul(M[i][0], a[0]) ^ GMul(M[i][1], a[1])
					^ GMul(M[i][2], a[2]) ^ GMul(M[i][3], a[3]);
				//轮密钥加
				state[i][j] ^= k[i][j];
				break;
			case 1:
				//直接使用原始字节作为S,将字节替换和行变换合并在一起
				a[0] = S[tmp[0][1]];
				a[1] = S[tmp[1][2]];
				a[2] = S[tmp[2][3]];
				a[3] = S[tmp[3][0]];

				//M矩阵的第二行乘以行移后tem矩阵的第二列
				state[i][j] = GMul(M[i][0], a[0]) ^ GMul(M[i][1], a[1])
					^ GMul(M[i][2], a[2]) ^ GMul(M[i][3], a[3]);
				//轮密钥加
				state[i][j] ^= k[i][j];
